...
def main():
    ...
    for i in range(FLAGS.num_structure_learning_iterations):
        data.train.positive_image_features = psdd.calculate_features(data.train.positive_images)
        data.train.negative_image_features = psdd.calculate_features(data.train.negative_images)
        data.test.positive_image_features = psdd.calculate_features(data.test.positive_images)
        data.test.negative_image_features = psdd.calculate_features(data.test.negative_images)
        if i % 10 == 0:
            psdd.learn_parameters(data.train, 3 * FLAGS.num_parameter_learning_iterations)
        else:
            psdd.learn_parameters(data.train, FLAGS.num_parameter_learning_iterations)

        with open(FLAGS.log_file, 'a+') as log_file:
            accuracy, precision, recall, f1 = psdd.calculate_accuracy_precision_recall_and_f1(data.train)
            print("Training accuracy: %f\tprecision: %f\trecall: %f\tf1: %f" %
                  (accuracy, precision, recall, f1))
            log_file.write("Training accuracy: %f\tprecision: %f\trecall: %f\tf1: %f\n" %
                           (accuracy, precision, recall, f1))
            accuracy, precision, recall, f1 = psdd.calculate_accuracy_precision_recall_and_f1(data.test)
            print("Testing accuracy: %f\tprecision: %f\trecall: %f\tf1: %f" %
                  (accuracy, precision, recall, f1))
            log_file.write("Testing accuracy: %f\tprecision: %f\trecall: %f\tf1: %f\n" %
                           (accuracy, precision, recall, f1))
            print("Num parameters: %d" % psdd.num_parameters)
            log_file.write("Num parameters: %d\n" % psdd.num_parameters)

        if accuracy > best_accuracy:
            best_accuracy = accuracy
            with open(FLAGS.prediction_file, 'w') as prediction_file:
                test_image_feature = psdd.calculate_features(data.test.images)
                predictions = psdd.predict(test_image_feature)
                prediction_file.write('\n'.join(str(p) for p in predictions))

        psdd.change_structure(data.train, FLAGS.depth, FLAGS.num_splits)
